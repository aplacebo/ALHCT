...
# logger = create_logger(output_dir='.', name=f"test")


_model_urls = {
    'flatten_swin_base_window7_224': 'https://cloud.tsinghua.edu.cn/f/7a9e5186bad04e7fb3a9/?dl=1',
    'flatten_swin_base_window12_384': 'https://cloud.tsinghua.edu.cn/f/0d0330cf2e5249f1abb6/?dl=1',
}
...
@register_model
def flatten_swin_b_w7_224(pretrained: bool = False, **kwargs):
    model = FLattenSwinTransformer(img_size=224, patch_size=4, 
    ...
    if pretrained:
        state_dict = torch.load('E:/~/crossswin/pretrained/ckpt_swin_b.pth', map_location='cpu')['model']
        #state_dict = torch.load('F:/~/aid0_2/aid__test.pth', map_location='cpu')['model']
        model = load_model(model, state_dict)
    return model

@register_model
def flatten_swin_b_w12_384(pretrained: bool = False, **kwargs):
    model = FLattenSwinTransformer(img_size=384, patch_size=4,  
                 embed_dim=128, depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32],
                 window_size=96, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 attn_drop_rate=0.,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 focusing_factor=3, kernel_size=5, attn_type='LLSS', **kwargs)
    if pretrained:
        state_dict = torch.load('E:/~/crossswin/pretrained/ckpt_swin_b_384.pth', map_location='cpu')['model']
        #state_dict = torch.load('F:/遥感数据集/权重文件/aid0_2/99_aid__test.pth', map_location='cpu')['model']
        model = load_model(model, state_dict)
    return model

if __name__ == '__main__':
    net = flatten_swin_b_w7_224(True, num_classes = 21)
    import numpy as np
    temp = torch.from_numpy(np.ones([1, 3, 224, 224], dtype=np.float32))
    out = net(temp)
    print(out.size())
